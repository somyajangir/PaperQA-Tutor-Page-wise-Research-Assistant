...
load_dotenv()
QG_BASE_MODEL_ID = "google/flan-t5-small"
OUTPUT_DIR = "outputs"
...
with col_work:
    ...
    with tutor_tab:
        if not has_pdf:
            ...
            # --- ADD this block right after the Key Terms panel and TTS button ---
            st.markdown("### Local Summary (fine‑tuned T5)")
            sum_c1, sum_c2 = st.columns([1, 5])
            with sum_c1:
               do_sum = st.button("▶ Summarize (T5)", key=f"btn_sum_{idx}", use_container_width=True)
            with sum_c2:
                 st.caption("Runs locally using your fine‑tuned FLAN‑T5 model (no API).")

            if do_sum:
              with st.spinner("Summarizing this page with fine‑tuned T5…"):
                   try:
